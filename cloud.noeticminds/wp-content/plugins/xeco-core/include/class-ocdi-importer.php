...
class GENIX_OCDI_Demo_Importer {
    ...
    public function import_files_config() {
		...
		$home_prevs = array(
			'genix_demo_home1' => array(
				...
				'preview_link' => 'https://xeco.themegenix.net',
			),
		);
    ...
    public function ocdi_after_import_setup( $selected_file ) {
        ...
        update_option( 'xeco_ocdi_importer_flash', true );
    }
    ...
    public function genix_ocdi_rewrite_flush() {
        ...
        if ( get_option( 'xeco_ocdi_importer_flash' ) == true ) {
            flush_rewrite_rules();
            delete_option( 'xeco_ocdi_importer_flash' );
        }
    }
}
